<template>
	<div id="classes-table">
		...
		<ClassesTableActionsBar @toggleShowFilter="toggleShowFilter" @search="search" :showFilter="showFilter" />
		<ClassesTableFilters
		...
		<div class="table-actions">
			<button id="btn-unselect-classes" @click="unselectClasses" class="btn">Bỏ chọn ({{ selectedClasses.length }} đã chọn)</button>

			<ClassesTablePaginationBar :page="page" @nextPage="nextPage" @prevPage="prevPage" />
		</div>
		...
		<div id="table-wrapper">
			<table id="table">
				<tr>
					<th></th>
					<th v-for="e in mapper" :key="e.field" v-show="displayFields.includes(e.field)">
						{{ e.name }}
					</th>
				</tr>
...
export default {
	...
	watch: {
		...
		selectedClasses(newVal) {
			...
			let removedClass = this.getSelectedClasses.find((c) => !newVal.includes(c));
			if (newClass) {
	...
	methods: {
		...
		nextPage() {
			if (this.page < Math.ceil(this.queriedClasses.length / this.itemsPerPage)) {
				this.page++;
			}
		},
		prevPage() {
